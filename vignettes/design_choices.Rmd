...
Cellpype objects have these slots:
  ...
  * **embed**
  * optional: *totalUMI*. Saves time if your data is huge and gene-wise HDF5Array
    (such as dataMS's ms_raw, for example).
  * **classes**: df with columns: class, parent.
    Order of classes does not change by recency, in contrast to rule
    (because why should it? :D).
  * **rules**: df with columns: class, feature, operator, threshold
    Most recent rule is *last* row in data.frame.
  * **.data**
    * original_object perhaps, such as a Seurat object? I am not planning 
      a function where cellpypes returns a Seurat object, so probably I won't
      need this.
    * computations: s, S, K of the most recent genes. 
      I think only `s` makes sense and I created the slot `obj$totalUMI` for it
      (crucial if obj$raw is in gene-wise HDF5Array format, which makes cell-wise 
      computations slow). See closed issue `precomputation function`.
      
      this gives us a large performance gain.
...
Don't forget `devtools::build_readme()` before 
pushing to git.
...
Whenever you write a new function:
  ...
  5. if you changed the readme file: `devtools::build_readme()`
...
Then again, perhaps it's better to return the plot itself. 
This way, you can use them with cowplot and patchwork.
